...
public class AILogic {
	...
	//There needs to be a clear distinction between what tiles are considered movement and which are considered attack 
	public static void performUnitBattle(Unit unitOfInterest, ActorRef out, GameState gameState) {
			...
			for (Tile attackTile: allTiles) {
				if (attackTile.getIsActionableTile() && !unitOfInterest.getHasAttacked() && attackTile.hasUnit()) {
					...
					//if the selected unit is able to kill the defending unit in 1 blow this happens
			...
			//if no easy kills are found this for loop is run instead for potential chip damage
			for (Tile attackTile: allTiles) {
				if (attackTile.getIsActionableTile() && !unitOfInterest.getHasAttacked() && attackTile.hasUnit()) {
					Unit unitToAttack = attackTile.getUnit();
					//if the selected unit will survive a counter attack then it will attack for the sake of attacking to chip the enemy unit
	...
	private static Tile findIfMoveExist(ArrayList<Tile> potentialTiles, Tile currentTile, int distanceThreshhold, ActorRef out) {
	...
	private static void summonAICard(ActorRef out, GameState gameState, Tile tile, Card card) {
			...
			unitSummon.setHealth(card.getHealth());
			unitSummon.setAttack(card.getAttack());
