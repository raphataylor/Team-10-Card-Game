...
public class TileClicked implements EventProcessor {
    ...
    public void processEvent(ActorRef out, GameState gameState, JsonNode message) {
        ...
        if (gameState.cardSelected) { // tile clicked after card is selected, summon that card unit on the tile
            ...
            //previouslySelectedTile is used for combat under the assumption it is referring to a unit thats
            //going to declare an attack
            gameState.isTileSelected = false;
            Game.summonUnit(out, gameState, tilex, tiley);
        } else if (tileSelected.hasUnit() && !gameState.isTileSelected) { // if the tile clicked has unit on it show valid moves
            ...
            return;
        } else if (gameState.currentSelectedUnit != null && selectedUnit == null) {// if a tile is clicked after a unit is clicked, move the
