...
//			}
	...
	*/
	...
	//[SPELL02] & [SPELL03]
	// Methods to handle highlighting of units when a spell is selected
	public static void highlightUnitsForSpell(ActorRef out, GameState gameState, Card spellCard) {
		// Adjusting to differentiate between spells for enemy and friendly units
		if (spellCard instanceof BeamShock || spellCard instanceof DarkTerminus || 
			spellCard instanceof TrueStrike || spellCard instanceof WraithlingSwarm) {
			highlightEnemyUnits(out, gameState);
		} else if (spellCard instanceof SundropElixir) {
			highlightFriendlyUnits(out, gameState);
		}
	}
	...
	public static void resetTileHighlighting(ActorRef out, GameState gameState) {
		for (Tile[] row : Game.getBoard().getTiles()) {
			for (Tile tile : row) {
				BasicCommands.drawTile(out, tile, 0); // 0 to remove highlight
				try { Thread.sleep(10); } catch (InterruptedException e) { e.printStackTrace(); }
			}
		}
	}
